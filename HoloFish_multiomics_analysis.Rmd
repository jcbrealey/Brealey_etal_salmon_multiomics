...
ggplot()+
  ...
  coord_equal()+
  labs(x = paste0("PC1 (",fa.pc1,"%)"), y = paste0("PC2 (",fa.pc2,"%)"), color = "FA batch", shape = "Feed type")+
...
ggplot()+
  ...
  scale_x_continuous(limits = c(-9,9)) + scale_y_continuous(limits = c(-9,9))+
  coord_equal()+
  labs(x = paste0("PC1 (",fa.pc1,"%)"), y = paste0("PC3 (",fa.pc3,"%)"), color = "Size class")+
  theme_bw() + theme(panel.grid = element_blank(), legend.position = "right",
                     axis.text = element_text(size = 12, colour = "black"), axis.title = element_text(size = 13), panel.border = element_rect(size = 1.2))
...
**PCA of fatty acid abundances** coloured processing batch (**A**) and size class (**B**). Shape indicates feed type in both. PCA on fatty acid profiles shows clear separation by feed type (PC1), fatty acid processing batch (PC2) and size class (PC3).

```{r}
per.dist.euc <- vegdist(per.pca.df[,which(names(per.pca.df) %in% analytes.per)], method = "euclidean")

adonis2(per.dist.euc ~ per.pca.df$Feed.Type +  per.pca.df$Size.class + per.pca.df$Tapeworm.bin + per.pca.df$Fatty_Acid_Batch, permutations = 999)
...
### Supp table: Differential abundance testing
...
**Table S1**. Associations (FDR < 0.2) between fish phenotypes and fatty acid relative abundances in muscle.
...
**Fig. S4**. Microbiota community composition of salmon gut content compared to salmon gut 1529 mucosa scrapes and feed pellet, determined by 16S metabarcoding. (**B**) Venn diagram of the 64 ASVs in **Fig. S4A** detected at least once within each sample type. The 16 ASVs unique to gut content samples included 14 low-abundant *Mycoplasmataceae* taxa and 2 *Alteromonadales* taxa.
...
### Supplementary ordination figure
...
**Fig. S2**. Alpha diversity (Hill’s Shannon index) of metagenomes, based on MAG abundance, stratified by cestode detection and size class.

**Table S4**. Linear model of metagenome alpha diversity differences among fish phenotypes.
...
**Table S5A.** Associations (FDR < 0.05) between fish phenotypes and metagenomes, based on MAG abundance (n = 392).
...
**Table S5B.** Associations (FDR < 0.05) between fish phenotypes and metagenomes, based on MAG presence/absence (n = 392).
...
gwas.mag05.res.all <- read.delim("output_GWAS/binPA_MAG05_Mycoplasma_sp_all.assoc.clean.txt")
...
**Fig. S6.** Ordination (PCA) of host gut epithelial transcriptomes. Individuals showed some variation by feed type on PC3 (**A**) and some variation by size class on PC4 (**B**). PCAs were based on normalised and transformed gene expression data. Based on PERMANOVA results (see below), feed type contributed to 2.7% of the variation in gene expression values (F = 9.63, p = 0.001) and size class contributed to 2.0% of the variation (F = 3.53, p = 0.001). Detailed results from the PERMANOVAs are presented in table S3.
...
**Fig. S7.** Ordination (PCA) of gut metabolomes, before (A, C, E) and after (B, D, F) removal of batch effects. Batch effects are a common problem in metabolomic studies and in this study, metabolome batch explains 11.4% of the variation in metabolite abundance before correction (F = 14.88, p = 0.001) (A), while after correction, batch does not significantly explain metabolite abundance variation (B). Feed type consistently explains 2.2-2.5% of the variation (F = 8.56, p = 0.001) (C–D) and size class explains 3.2-3.6% of the variation (F = 6.35, p = 0.001). Detailed results from the PERMANOVAs (see below) are presented in table S3.

```{r}
# permanovas
mbol.before.dist <- vegdist(t(assay(mbol.met[["norm_imputed"]])), method = "euclidean")

adonis2(mbol.before.dist ~ Mbol.batch + Feed.Type + Size.class, mbol.meta[match(mbol.meta$Fish.ID,colnames(assay(mbol.met[["norm_imputed"]]))),],
        permutations = 999, method = "euclidean")

mbol.after.dist <- vegdist(t(mbol.nobatch), method = "euclidean")

adonis2(mbol.after.dist ~ Mbol.batch + Feed.Type + Size.class, mbol.meta[match(mbol.meta$Fish.ID,colnames(mbol.nobatch)),],
        permutations = 999, method = "euclidean")
